...
- **Política de Cero Stubs Silenciosos (B-001)**: Los orquestadores y proveedores N1 (`PmcOrchestrator`, `PlaneacionOrchestrator`, `PaecOrchestrator`, `CartografiaContextProvider`) tienen estrictamente prohibido simular éxito con buffers vacíos (`Buffer.from('')`) o retornos ficticios en métodos no migrados. Cualquier método en desarrollo lanza `501 NOT_IMPLEMENTED` de forma ruidosa.
...
- **Resiliencia de Ingesta y Proveedores de IA (Incidente 503 / 504)**:
  ...
  - **Módulo Transversal de Resiliencia (`src/lib/ai-resilience.ts`)**: Centraliza `withTimeoutBudget`, `isUpstreamAIError` y `AI_OUTAGE_USER_MESSAGE`, desacoplando completamente los orquestadores N1 entre sí (evitando imports cruzados PAEC/Cartografía → PMC). Soporta detección estricta ampliada contra caídas HTTP 503 crudas y Service Unavailable.
