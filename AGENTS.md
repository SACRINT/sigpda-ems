...
- **Resiliencia de Ingesta y Proveedores de IA (Incidente 503 / 504)**:
  ...
  - **Presupuesto de Tiempo y Cliente Seguro**: Se aplica un límite preventivo (`withTimeoutBudget`, 90s) para evitar timeouts sucios de 120s de Vercel, y los clientes web verifican `content-type` antes de invocar `res.json()`, eliminando excepciones `Unexpected token 'A'`.
