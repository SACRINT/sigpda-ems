...
- **Política de Cero Stubs Silenciosos (B-001)**: Los orquestadores N1 (`PmcOrchestrator`, `PlaneacionOrchestrator`, `PaecOrchestrator`) tienen estrictamente prohibido simular éxito con buffers vacíos (`Buffer.from('')`) o retornos ficticios en métodos no migrados. Cualquier método en desarrollo lanza `501 NOT_IMPLEMENTED` de forma ruidosa.
