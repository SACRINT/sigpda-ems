...
async function main() {
  const planningId = '215643ee-47ad-436a-85df-ea0c8ca39ca4'; // Pensamiento Matemático I BGE
  const blockIndex = 0;

  console.log('================================================================');
  console.log('🚀 INICIANDO PILOTO BGE: Bloque I de Pensamiento Matemático (12 horas)');
  console.log('================================================================');
  ...
  console.log(`\n✅ Libro BGE generado con éxito en ${elapsedSec}s!`);
  ...
  console.log('\n📄 Renderizando DOCX BGE...');
  const docxBuffer = await renderWorkbookToDocx(workbook, planning);
  const docxPath = path.resolve('./scratch/Piloto_BGE_Bloque_I_Matematicas.docx');
  fs.writeFileSync(docxPath, docxBuffer);
  console.log(`✅ DOCX BGE guardado en: ${docxPath} (${(docxBuffer.length / 1024).toFixed(1)} KB)`);

  console.log('\n📑 Renderizando PDF BGE...');
  const pdfBuffer = await renderWorkbookToPdf(workbook, planning);
  const pdfPath = path.resolve('./scratch/Piloto_BGE_Bloque_I_Matematicas.pdf');
  fs.writeFileSync(pdfPath, pdfBuffer);
  console.log(`✅ PDF BGE guardado en: ${pdfPath} (${(pdfBuffer.length / 1024).toFixed(1)} KB)`);

  // Guardar JSON
  const jsonPath = path.resolve('./scratch/Piloto_BGE_Bloque_I_Matematicas.json');
  fs.writeFileSync(jsonPath, JSON.stringify(workbook, null, 2), 'utf-8');

  console.log('\n🔍 Auditoría de Misiones BGE:');
...
main().catch((err) => {
  console.error('❌ Error fatal en ejecución Piloto BGE:', err);
  process.exit(1);
});
