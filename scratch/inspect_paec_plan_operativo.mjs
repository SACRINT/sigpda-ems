...
async function inspectPaec() {
  ...
  console.log(`Loaded PAEC PDF with ${doc.numPages} pages.`);
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    if (/plan\s+operativo/i.test(text) || /operativo/i.test(text) || /asignatura/i.test(text) || /disciplina/i.test(text) || /cronograma/i.test(text)) {
      console.log(`\n=== PAGE ${p} ===`);
...
inspectPaec().catch(console.error);
