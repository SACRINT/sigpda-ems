...
# Helper to parse table structure per PDF
def parse_pdf_map(f):
    ...
    # We extract all table cells
    cells_by_page = []
    for p in mapa_pages:
        tabs = doc[p].find_tables()
        for tab in tabs.tables:
            for row in tab.extract():
                cells_by_page.append([clean(c) for c in row if c is not None])
                
    return curr_name, mapa_pages, cells_by_page

print("Testing table parser...")
for f in sorted(PDF_TO_CURRICULUM.keys()):
    curr, pages, cells = parse_pdf_map(f)
    print(f"{curr}: {len(cells)} rows across pages {[p+1 for p in pages]}")
