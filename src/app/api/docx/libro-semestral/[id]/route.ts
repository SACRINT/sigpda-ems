...
export async function GET(
  ...
  try {
    ...
    const { docx } = await compileSemestralWorkbook(id, planning, { format: 'docx' });
    if (!docx) {
      throw new Error('No se pudo generar el archivo DOCX semestral');
    }
    const cleanUac = (planning.uacName || 'UAC').replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ_ -]/g, '').trim();
    const filename = `Libro_Maestro_Semestral_${cleanUac}.docx`;
