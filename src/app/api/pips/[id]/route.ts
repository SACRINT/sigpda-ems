  ...
  try {
    ...
    const [project] = await db`
      UPDATE pips_projects SET
        ...
        momento3_ubicar            = COALESCE(${body.momento3_ubicar != null ? JSON.stringify(body.momento3_ubicar) : null}::jsonb, momento3_ubicar),
        momento4_analizar          = COALESCE(${body.momento4_analizar != null ? JSON.stringify(body.momento4_analizar) : null}::jsonb, momento4_analizar),
        momento5_decidir           = COALESCE(${body.momento5_decidir != null ? JSON.stringify(body.momento5_decidir) : null}::jsonb, momento5_decidir),
        memoria_pedagogica         = COALESCE(${body.memoria_pedagogica != null ? JSON.stringify(body.memoria_pedagogica) : null}::jsonb, memoria_pedagogica),
